...
def compute_track_y(segments, z):
    """Replicate _build_segments logic exactly."""
    current_y = 0.0
    for seg in segments:
        z0 = seg.get("z0", 0.0)
        z1 = seg.get("z1", -100.0)
        ...
        # Check if z falls within this segment
        if z0 < z1:
            in_seg = z0 <= z <= z1
        else:
            in_seg = z1 <= z <= z0
        ...
        # Update current_y for next segment (same logic as _build_segments)
        if abs(ramp) > 0.01:
...
def fix_hoops_in_level(src, key):
    ...
    # Also parse obstacles to find existing hoops
    ...
    for obs in obstacles:
        ...
        if track_y is None:
            # In a gap — find nearest segment center
            nearest_y = 0.0
            best_dist = float('inf')
            for seg in segments:
        ...
        if abs(new_y - old_y) > 0.1:
            ...
            # Do a targeted replacement inside the level_block
