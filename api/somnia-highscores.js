/**
 * Somnia global high scores — proxies to Google Sheets (Apps Script).
 *
 * GET  → { scores: [...] }  top 100
 * POST → { ok, rank, inTop, scores }  submit a run
 *
 * Vercel environment variables:
 *   SOMNIA_HIGHSCORES_SCRIPT_URL  — deployed Apps Script web app URL
 *   SOMNIA_HIGHSCORES_API_SECRET — must match API_SECRET in the script (default: studentsfirst)
 *
...
async function fetchScriptJson(url, options) {
  ...
  try {
    ...
    console.error("Somnia high scores script non-JSON response:", text.slice(0, 200));
...
export async function POST(request) {
  ...
  try {
    ...
    console.error("Somnia high scores POST proxy error:", e.message);
